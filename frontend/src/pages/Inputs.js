...
const Inputs = () => {
  ...
  const fetchProducts = () => {
    return new Promise((resolve, reject) => {
      axios.get("http://localhost:9000/product").then(({ data }) => {
        setProducts(data);
        resolve();
      });
    });
...
const InputsHandler = (props) => {
  ...
  const fetchInputs = () => {
    return new Promise((resolve, reject) => {
      axios.get(`http://localhost:9000/raw_material/by_product/${productId}`).then(({ data }) => {
        setRows(data);
        resolve();
      });
    });
  ...
  const fetchMaterials = () => {
    return new Promise((resolve, reject) => {
      axios.get(`http://localhost:9000/raw_material`).then(({ data }) => {
        setMaterials(data);
        resolve();
      });
    });
  ...
  const deleteRowHandler = (row) => {
    ...
    axios.delete(`http://localhost:9000/input`, { 
          data:
            {
              product: Number(productId),
